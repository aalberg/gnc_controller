...
static uint8_t cfg_descriptor[] = {
  ...
  0, //iInterface
    /* INTERFACE 0, ENDPOINT 0 BEGIN */
    7, //bLength
    5, //bDescriptorType,
    0x81, //bEndpointAddress,
    0x02, //bmAttributes (bulk endpoint, ignored by host?)
    ENDP1_SIZE, 0x00, //wMaxPacketSize,
    0, //bInterval
    /* INTERFACE 0, ENDPOINT 0 END */
...
// Endpoint 0 setup handler. Called from endpoint 0 handler.
...
// Table of endpoint handler callbacks.
...
void USBOTG_IRQHandler(void)
{
    ...
    if (status & USB_ISTAT_USBRST_MASK)
    {
        ...
        // Initialize endpoint 0 ping-pong buffers. (This is the only endpoint used.
    ...
    if (status & USB_ISTAT_TOKDNE_MASK)
    {
        ...
        stat = USB0_STAT;
        endpoint = stat >> 4;
        // Call the appropriate handler to figure out what to do next.
